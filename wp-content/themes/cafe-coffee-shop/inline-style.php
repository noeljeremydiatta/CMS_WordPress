<?php
	...
	if($vw_bakery_second_color != false){
		$vw_bakery_custom_css .='.more-btn a:hover, .scrollup i, .pagination span, .pagination a, input[type="submit"], #comments a.comment-reply-link:hover, .woocommerce #respond input#submit:hover, .woocommerce a.button:hover, .woocommerce button.button:hover, .woocommerce input.button:hover, .woocommerce #respond input#submit.alt:hover, .woocommerce a.button.alt:hover, .woocommerce button.button.alt:hover, .woocommerce input.button.alt:hover, .products li:hover span.onsale, .carousel-control-prev-icon i:hover, .carousel-control-next-icon i:hover, #sidebarcustom-social-icons i:hover, #footer .custom-social-icons i:hover{';
			$vw_bakery_custom_css .='background-color: '.esc_html($vw_bakery_second_color).';';
		$vw_bakery_custom_css .='}';
	}
	if($vw_bakery_second_color != false){
		$vw_bakery_custom_css .='#heade-top i, .post-main-box:hover h2 a, .post-main-box:hover a, .main-navigation a:hover, #footer li a:hover, a.rsswidget, .main-navigation ul.sub-menu a:hover, .copyright a:hover, .entry-content a, .sidebar .textwidget p a, .textwidget p a, #comments p a, .woocommerce ul.products li.product .price, .woocommerce div.product p.price, .woocommerce div.product span.price, .product_meta a, span.tagged_as a,#sidebar a.custom_read_more:hover{';
			$vw_bakery_custom_css .='color: '.esc_html($vw_bakery_second_color).';';
		$vw_bakery_custom_css .='}';
	}
	if($vw_bakery_second_color != false){
		$vw_bakery_custom_css .='.more-btn a:hover{';
