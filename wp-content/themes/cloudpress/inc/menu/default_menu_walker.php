<?php 
function cloudpress_page_menu_args( $args ) {
	if ( ! isset( $args['show_home'] ) )
		$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'cloudpress_page_menu_args' );
function cloudpress_fallback_page_menu( $args = array() ) {
	...
	if ( ! empty($args['show_home']) ) {
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = esc_html__('Home','cloudpress');
		else
			$text = $args['show_home'];
		$class = '';
	...
	$list_args['title_li'] = '';
	$list_args['walker'] = new cloudpress_walker_page_menu;
...
class cloudpress_walker_page_menu extends Walker_Page{
